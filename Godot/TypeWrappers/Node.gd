...
func _ready():
	print( setWrapper._array )
	setWrapper.connect("changed", self, "printSetChanged")

	setWrapper.add( [4] )
	assert( setWrapper._array == [1,2,3,4] )
	setWrapper.remove( [3] )
	assert( setWrapper._array == [1,2,4] )
	setWrapper.remove( [8] )
	assert( setWrapper._array == [1,2,4] )
	setWrapper.add( [7] )
	assert( setWrapper._array == [1,2,4,7] )
	setWrapper.reset( [4,3,6] )
	assert( setWrapper._array == [4,3,6] )
	assert( setChanges == 4 )

	print(mapWrapper._dict )
	mapWrapper.connect("changed", self, "printMapChanged")

	mapWrapper.add( {'s':1, 8:'e'} )
	assert( mapWrapper._dict.hash() == {1:2, 2:3,'s':1, 8:'e'}.hash() )
	mapWrapper.add( {'s':'p', 8:5} )
	assert( mapWrapper._dict.hash() == {1:2, 2:3,'s':1, 8:'e'}.hash() )
	mapWrapper.reset( {1:2, 2:3, 3:4} )
	assert( mapWrapper._dict.hash() == {1:2, 2:3, 3:4}.hash() )
	mapWrapper.remove( {2:3} )
	assert( mapWrapper._dict.hash() == {1:2, 3:4}.hash() )
	mapWrapper.remove( {2:3} )
	assert( mapWrapper._dict.hash() == {1:2, 3:4}.hash() )
	mapWrapper.replace( {1:'o', 6:4} )
	assert( mapWrapper._dict.hash() == {1:'o', 3:4, 6:4}.hash() )
	assert( mapChanges == 4 )
...
func getSet():
	return setWrapper._array

func getMap():
	return mapWrapper._dict


func makeGuard( node : Node ):
	var _guard = NodeGuardGd.new( node )
